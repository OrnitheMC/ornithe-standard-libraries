...
public class WrappedIdRegistry<T> implements ClearableRegistry<T> {
	...
	public static <T> WrappedIdRegistry<T> of(ResourceKey<? extends Registry<T>> key, IdRegistry<Identifier, T> registry) {
		return new WrappedIdRegistry<>(key.identifier(), registry);
	}

	final NamespacedIdentifier identifier;
	final IdRegistry<Identifier, T> registry;

	private int nextId;
	private boolean frozen;

	WrappedIdRegistry(NamespacedIdentifier identifier, IdRegistry<Identifier, T> registry) {
	...
	private void valueRegistered(int id, Object key, Object value) {
		if (this.frozen) {
	...
	public <V extends T> V register(int id, ResourceKey<T> key, V value) {
		this.registry.register(id, this.deserializeKey(key.identifier()), value);
		return value;
	}
	...
	public T get(ResourceKey<T> key) {
		return this.registry.get(this.deserializeKey(key.identifier()));
	}

	@Override
	public T get(NamespacedIdentifier identifier) {
		return this.registry.get(this.deserializeKey(identifier));
	}
	...
	public Set<ResourceKey<T>> keySet() {
		return this.registry.entrySet().stream().map(this::serializeKey).map(this::resourceKey).collect(Collectors.toSet());
	}

	@Override
	public Set<NamespacedIdentifier> identifierSet() {
		return this.registry.entrySet().stream().map(this::serializeKey).collect(Collectors.toSet());
	}
