...
public interface BlockExtensionImpl extends BlockExtension {
	...
	@Override
	default MapColor getMapColor(BlockState state) {
		throw new AbstractMethodError();
	}
	...
	@Override
	default Sprite getSprite(BlockState state, Direction face) {
		throw new AbstractMethodError();
	}
	...
	@Override
	default int getSignal(WorldView world, BlockPos pos, BlockState state, Direction dir) {
		throw new AbstractMethodError();
	}

	@Override
	default int getAnalogSignal(World world, BlockPos pos, BlockState state) {
		throw new AbstractMethodError();
	}
	...
	@Override
	default int getDirectSignal(WorldView world, BlockPos pos, BlockState state, Direction dir) {
		throw new AbstractMethodError();
	}
	...
	@Override
	default void addCollisions(World world, BlockPos pos, BlockState state, Box shape, List<Box> collisions, Entity entity) {
		throw new AbstractMethodError();
	}
	...
	@Override
	default boolean doEvent(World world, BlockPos pos, BlockState state, int type, int data) {
		throw new AbstractMethodError();
	}
	...
	@Override
	default boolean use(World world, BlockPos pos, BlockState state, PlayerEntity player, Direction face, float faceX, float faceY, float faceZ) {
		throw new AbstractMethodError();
	}
