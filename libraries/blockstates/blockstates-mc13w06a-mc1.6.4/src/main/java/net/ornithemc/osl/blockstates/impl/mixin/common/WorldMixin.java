...
public class WorldMixin implements WorldExtension {
	...
	@Shadow
	private void updateNeighborComparators(int x, int y, int z, int block) { }
	...
	public boolean setBlockState(int x, int y, int z, BlockState state, int flags) {
		...
		if ((flags & SetBlockFlags.NOTIFY_LISTENERS) != 0 && (!this.isMultiplayer || (flags & SetBlockFlags.NO_REDRAW) == 0)) {
		...
		if (!this.isMultiplayer && (flags & SetBlockFlags.NOTIFY_NEIGHBORS) != 0) {
			...
			if (replaced.isAnalogSignalSource()) {
				this.updateNeighborComparators(x, y, z, replacedBlock.id);
			}
		}
