...
public interface BlockExtension {
	...
	Sprite getSprite(BlockState state, Direction face);

	int getLightColor(WorldView world, BlockPos pos);

	float getAmbientOcclusionLight(WorldView world, BlockPos pos);

	int getSignal(WorldView world, BlockPos pos, BlockState state, Direction dir);

	int getAnalogSignal(World world, BlockPos pos, BlockState state);

	float getMiningTime(World world, BlockPos pos);

	float getMiningSpeed(PlayerEntity player, World world, BlockPos pos);

	int getDirectSignal(WorldView world, BlockPos pos, BlockState state, Direction dir);
	...
	void addCollisions(World world, BlockPos pos, BlockState state, Box shape, List<Box> collisions, Entity entity);
	...
	boolean doEvent(World world, BlockPos pos, BlockState state, int type, int data);
	...
	void dropItems(World world, BlockPos pos, BlockState state, int fortuneLevel);

	void dropItems(World world, BlockPos pos, BlockState state, float luck, int fortuneLevel);

	boolean use(World world, BlockPos pos, BlockState state, PlayerEntity player, Direction face, float faceX, float faceY, float faceZ);
