...
public class NetworkOptionSerializers {
	...
	public static final NetworkOptionSerializer<IdentifierOption> IDENTIFIER = register(IdentifierOption.class, Identifier.class);
	...
	public static final NetworkOptionSerializer<ListOption> LIST = register(ListOption.class, new NetworkOptionSerializer<ListOption>() {
		...
		public void serialize(ListOption option, SerializationSettings settings, PacketByteBuf buffer) throws IOException {
			NetworkSerializers.Lists.serialize(option, buffer);
		}

		@Override
		@SuppressWarnings("unchecked")
		public void deserialize(ListOption option, SerializationSettings settings, PacketByteBuf buffer) throws IOException {
			NetworkSerializers.Lists.deserialize(option, buffer);
		}
	});
	@SuppressWarnings("rawtypes")
	public static final NetworkOptionSerializer<MapOption> MAP = register(MapOption.class, new NetworkOptionSerializer<MapOption>() {
		...
		@Override
		@SuppressWarnings("unchecked")
		public void serialize(MapOption option, SerializationSettings settings, PacketByteBuf buffer) throws IOException {
			NetworkSerializers.Maps.serialize(option, buffer);
		}

		@Override
		@SuppressWarnings("unchecked")
		public void deserialize(MapOption option, SerializationSettings settings, PacketByteBuf buffer) throws IOException {
			NetworkSerializers.Maps.deserialize(option, buffer);
		}
	});
	...
	public static <T, O extends BaseOption<T>> NetworkOptionSerializer<O> register(Class<O> optionType, Class<T> valueType) {
		return register(optionType, new NetworkOptionSerializer<O>() {
			...
			@Override
			public void serialize(O option, SerializationSettings settings, PacketByteBuf buffer) throws IOException {
				NetworkSerializers.get(valueType).serialize(option.get(), buffer);
			}

			@Override
			public void deserialize(O option, SerializationSettings settings, PacketByteBuf buffer) throws IOException {
				option.set(NetworkSerializers.get(valueType).deserialize(buffer));
			}
		});
	}
