...
public final class ClientPlayNetworkingImpl {
	...
	private static <T extends PacketPayload> void registerListener(Channel channel, Supplier<T> initializer, ClientPacketListener.Payload<T> listener, boolean async) {
		registerListenerImpl(channel, new BufferListener() {
			...
			@Override
			public boolean handle(Minecraft minecraft, ClientPlayNetworkHandler handler, PacketByteBuf buffer) throws IOException {
	...
	public static boolean handle(Minecraft minecraft, ClientPlayNetworkHandler handler, Packet packet) {
	...
	private static void sendInternal(Channel channel, IOConsumer<PacketByteBuf> writer) {
		sendPacket(makePacket(channel, writer));
	}

	private static void sendInternal(Channel channel, PacketByteBuf buffer) {
	...
	private interface BufferListener {
		...
		boolean handle(Minecraft minecraft, ClientPlayNetworkHandler handler, PacketByteBuf buffer) throws IOException;
