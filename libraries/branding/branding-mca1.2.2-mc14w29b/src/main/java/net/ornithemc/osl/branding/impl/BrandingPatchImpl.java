	...
	public void initClient() {
		LaunchEvents.PARSE_RUN_ARGS.register(new OptionsConsumer() {
			...
			public void defineOptions(OptionParser parser) {
				Map<String, OptionSpec<?>> specs = parser.recognizedOptions();

				if (!specs.containsKey(Constants.VERSION)) {
					// needed so that --version is not covered by --versionType
					parser.accepts(Constants.VERSION).withRequiredArg();
				}
